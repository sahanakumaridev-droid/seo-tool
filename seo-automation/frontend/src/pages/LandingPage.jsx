...
const C = {
  ...
  brand: '#38BDF8', violet: '#818CF8', green: '#34D399', amber: '#FBBF24',
  t1: '#F1F5F9', t2: '#94A3B8', t3: '#64748B',
  grad: 'linear-gradient(135deg,#38BDF8,#6366F1)',
}

const FEATURES = [
  { Icon: FileText,  accent: '#38BDF8', title: 'AI Bulk Content',      desc: 'Generate 50+ SEO-optimized pages in seconds using GPT-4.' },
  { Icon: Globe,     accent: '#818CF8', title: 'WordPress Publishing', desc: 'Auto-publish with RankMath, Yoast, or AIOSEO. Images, categories, tags included.' },
  { Icon: Image,     accent: '#A78BFA', title: 'AI Image Generation',  desc: 'Auto-generate and attach relevant images using DALL-E 3, Unsplash, or Pexels.' },
  { Icon: Share2,    accent: '#F472B6', title: 'Social Automation',    desc: 'Share to Facebook, Twitter/X, LinkedIn, Instagram with captions and hashtags.' },
  { Icon: Users,     accent: '#34D399', title: 'Lead Capture',         desc: 'Capture and manage leads from Bark.com, Thumbtack, and your website.' },
  { Icon: BarChart2, accent: '#FBBF24', title: 'SEO Analytics',        desc: 'Track keyword rankings, organic traffic, and competitor gaps.' },
]

const STEPS = [
  { n: '01', title: 'Set Your Business',     desc: 'Enter business type, location, target keywords. Choose cities and industry.', accent: '#38BDF8' },
  { n: '02', title: 'AI Generates Content',  desc: 'GPT-4 creates unique, SEO-optimized pages for every city with titles, meta, FAQs, schema.', accent: '#818CF8' },
  { n: '03', title: 'Auto-Publish & Share',  desc: 'Pages publish to WordPress automatically. Social posts go out to all platforms instantly.', accent: '#34D399' },
]

const TESTIMONIALS = [
  { name: 'Marcus T.', role: 'Plumbing Business Owner', text: 'Generated 50 city pages in under 2 minutes. Our organic traffic doubled in 60 days.' },
  { name: 'Sarah K.', role: 'Digital Marketing Agency', text: 'We use this for all our local SEO clients. The WordPress auto-publish saves us 10+ hours a week.' },
  { name: 'James R.', role: 'HVAC Contractor',          text: 'The lead capture from Bark and Thumbtack is a game changer. Everything in one place.' },
]
    ...
    <button onClick={onClick} onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)}
      style={{
        ...
        ...(primary ? {
          ...
          boxShadow: hov ? '0 8px 28px rgba(56,189,248,0.4)' : '0 4px 16px rgba(56,189,248,0.25)',
        } : {
...
function LoginDialog({ onClose, onLogin }) {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 999, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={onClose}>
      ...
      <div style={{ position: 'relative', width: '100%', maxWidth: 420, background: '#fff', borderRadius: 20, boxShadow: '0 25px 60px rgba(0,0,0,0.6)', overflow: 'hidden' }} onClick={e => e.stopPropagation()}>
        <button onClick={onClose} style={{ position: 'absolute', top: 16, right: 16, width: 32, height: 32, borderRadius: 8, background: 'rgba(0,0,0,0.05)', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#6b7280', transition: 'all 0.15s' }}
          onMouseEnter={e => { e.currentTarget.style.background = 'rgba(0,0,0,0.1)'; e.currentTarget.style.color = '#111827' }}
        ...
        <div style={{ padding: '40px 36px 36px' }}>
          <div style={{ textAlign: 'center', marginBottom: 28 }}>
            <div style={{ width: 52, height: 52, borderRadius: 14, background: C.grad, display: 'inline-flex', alignItems: 'center', justifyContent: 'center', marginBottom: 14, boxShadow: '0 8px 24px rgba(56,189,248,0.3)' }}>
              <Zap size={26} color="white" />
            </div>
            <h2 style={{ fontSize: 24, fontWeight: 800, color: '#111827', margin: '0 0 6px', letterSpacing: '-0.5px' }}>Welcome to ZEORBIT</h2>
            <p style={{ fontSize: 14, color: '#6b7280', margin: 0 }}>Sign in to your SEO dashboard</p>
          </div>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            <div>
              ...
              <input type="text" value={form.username} onChange={e => setForm(f => ({ ...f, username: e.target.value }))} placeholder="admin" autoComplete="username"
                style={{ width: '100%', padding: '11px 14px', fontSize: 14, borderRadius: 10, border: '1.5px solid #e5e7eb', background: '#f9fafb', color: '#111827', outline: 'none', boxSizing: 'border-box', transition: 'all 0.15s' }}
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                <input type={showPw ? 'text' : 'password'} value={form.password} onChange={e => setForm(f => ({ ...f, password: e.target.value }))} placeholder="••••••••" autoComplete="current-password"
                  style={{ width: '100%', padding: '11px 42px 11px 14px', fontSize: 14, borderRadius: 10, border: '1.5px solid #e5e7eb', background: '#f9fafb', color: '#111827', outline: 'none', boxSizing: 'border-box', transition: 'all 0.15s' }}
                  onFocus={e => { e.target.style.borderColor = '#1877f2'; e.target.style.boxShadow = '0 0 0 3px rgba(24,119,242,0.1)'; e.target.style.background = '#fff' }}
                  onBlur={e => { e.target.style.borderColor = '#e5e7eb'; e.target.style.boxShadow = 'none'; e.target.style.background = '#f9fafb' }}
                />
            ...
            <button type="submit" disabled={loading || !form.username || !form.password}
              style={{
                ...
                boxShadow: '0 4px 14px rgba(56,189,248,0.35)', transition: 'all 0.15s', marginTop: 4,
              }}>
          ...
          <div style={{ marginTop: 20, paddingTop: 20, borderTop: '1px solid #e5e7eb', textAlign: 'center' }}>
            <p style={{ fontSize: 12, color: '#9ca3af', margin: 0 }}>
              Demo: <span style={{ fontFamily: 'monospace', background: '#f3f4f6', padding: '2px 6px', borderRadius: 4, color: '#111827', fontWeight: 600 }}>admin / admin</span>
            </p>
          </div>
        </div>
      </div>
...
export default function LandingPage({ onLogin }) {
  ...
  return (
    <div style={{ minHeight: '100vh', background: C.bg, color: C.t1, fontFamily: "'Inter',-apple-system,sans-serif", overflowX: 'hidden' }}>
      ...
      <nav style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
          <div style={{ width: 36, height: 36, borderRadius: 10, background: C.grad, display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 0 20px rgba(56,189,248,0.4)' }}>
            <Zap size={18} color="white" />
          </div>
          <span style={{ fontWeight: 900, fontSize: 18, letterSpacing: '-0.5px' }}>ZEORBIT</span>
          <span style={{ fontSize: 11, padding: '2px 8px', borderRadius: 20, background: 'rgba(56,189,248,0.1)', color: C.brand, border: `1px solid rgba(56,189,248,0.2)`, marginLeft: 4 }}>SEO AI</span>
        </div>
      ...
      <section style={{ paddingTop: 140, paddingBottom: 100, textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', top: 0, left: '50%', transform: 'translateX(-50%)', width: 700, height: 400, borderRadius: '50%', background: 'radial-gradient(ellipse,rgba(56,189,248,0.15) 0%,rgba(99,102,241,0.08) 50%,transparent 70%)', filter: 'blur(60px)', pointerEvents: 'none' }} />

        <div style={{ position: 'relative', maxWidth: 800, margin: '0 auto', padding: '0 24px' }}>
          <div style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 14px', borderRadius: 20, background: 'rgba(56,189,248,0.08)', border: `1px solid rgba(56,189,248,0.2)`, color: C.brand, fontSize: 13, fontWeight: 600, marginBottom: 24 }}>
            <Zap size={13} /> AI-Powered Local SEO Automation
          </div>

          <h1 style={{ fontSize: 58, fontWeight: 900, lineHeight: 1.1, margin: '0 0 20px', letterSpacing: '-2px' }}>
            Dominate Local Search<br />
            <span style={{ background: C.grad, WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>in Every City</span>
          </h1>

          <p style={{ fontSize: 18, color: C.t2, maxWidth: 560, margin: '0 auto 40px', lineHeight: 1.7 }}>
            Generate 50+ SEO-optimized pages, auto-publish to WordPress, share to social media, and capture leads — all from one AI-powered platform.
          </p>
      ...
      <section id="how-it-works" style={{ padding: '80px 24px' }}>
        <div style={{ maxWidth: 960, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            ...
            <p style={{ color: C.t2, fontSize: 15 }}>Three steps to dominate local SEO in any city</p>
          </div>
      ...
      <section id="reviews" style={{ padding: '80px 24px' }}>
        <div style={{ maxWidth: 900, margin: '0 auto' }}>
          <div style={{ textAlign: 'center', marginBottom: 56 }}>
            <h2 style={{ fontSize: 34, fontWeight: 800, margin: '0 0 10px', letterSpacing: '-0.5px' }}>Trusted by Local Businesses</h2>
            <p style={{ color: C.t2, fontSize: 15 }}>See what our customers are saying</p>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
            {TESTIMONIALS.map(t => (
      ...
      <section style={{ padding: '100px 24px', textAlign: 'center', position: 'relative', overflow: 'hidden' }}>
        <div style={{ position: 'absolute', inset: 0, background: 'radial-gradient(ellipse at center,rgba(99,102,241,0.1) 0%,transparent 65%)', pointerEvents: 'none' }} />
        <div style={{ position: 'relative', maxWidth: 560, margin: '0 auto' }}>
          <h2 style={{ fontSize: 40, fontWeight: 900, margin: '0 0 16px', letterSpacing: '-1px' }}>Ready to Dominate Local SEO?</h2>
          <p style={{ color: C.t2, fontSize: 16, margin: '0 0 36px', lineHeight: 1.6 }}>Start generating AI-powered content for every city in your market today.</p>
